import type { UiAccountMeta, UiBackupStatus, UiKeyringMeta } from "@arx/core/ui";
...
export const createUiKeyringBackupStatusQueryOptions = () => ({
  ...
  queryFn: (): Promise<UiBackupStatus> => uiClient.keyrings.getBackupStatus(),
  staleTime: 30_000,
});
...
export const refreshUiKeyringBackupStatusIntoCache = async (queryClient: QueryClient): Promise<UiBackupStatus> => {
  const status = await uiClient.keyrings.getBackupStatus();
