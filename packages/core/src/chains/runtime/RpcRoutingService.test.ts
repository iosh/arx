...
import { InMemoryRpcRoutingService } from "./RpcRoutingService.js";
...
describe("InMemoryRpcRoutingService", () => {
  it("rotates endpoints and records failures", () => {
    ...
    const routingService = new InMemoryRpcRoutingService({
      messenger,
    ...
    routingService.onRpcEndpointChanged((change) => endpointChanges.push(change));
    routingService.onRpcHealthChanged((update) => healthUpdates.push(update));
    ...
    routingService.reportRpcOutcome(metadata.chainRef, failure);

    expect(routingService.getState().rpc[metadata.chainRef]?.activeIndex).toBe(1);
  ...
  it("clears cooldown and logs recovery on success", () => {
    ...
    routingService.reportRpcOutcome(metadata.chainRef, {
      success: false,
      error: { message: "temporary" },
    });

    now = 11_000;
    routingService.reportRpcOutcome(metadata.chainRef, { success: true });
  ...
  it("clamps routing index when registry metadata changes", () => {
    ...
    routingService.onChainConfigChanged((payload) => updates.push(payload));
    ...
    routingService.replaceState(
      runtimeFromMetadata(updated, {
        rpc: { [updated.chainRef]: { activeIndex: 1, strategy: { id: "round-robin" } } },
      }),
    );

    expect(routingService.getState().rpc[metadata.chainRef]?.activeIndex).toBe(0);
