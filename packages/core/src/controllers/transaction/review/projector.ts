...
const deriveReviewState = (meta: TransactionMeta | undefined): TransactionReviewState => {
  if (!meta) {
    return {
      status: "preparing",
      revision: 0,
...
export const projectTransactionReviewState = (transaction: TransactionMeta | undefined) => {
  const reviewState = deriveReviewState(transaction);
  const warnings = (transaction?.warnings ?? []).flatMap((warning) => {
    const projected = toReviewMessage(warning);
    return projected ? [projected] : [];
  });
  const blockingIssue = reviewState.status === "ready" ? toReviewMessage(transaction?.issues[0]) : null;

  return {
    reviewState,
    warnings,
    blockingIssue,
  } satisfies Pick<SendTransactionApprovalReview, "reviewState" | "warnings" | "blockingIssue">;
};
...
export const buildSendTransactionApprovalReview = (args: {
  ...
  return {
    ...projectTransactionReviewState(args.transaction),
