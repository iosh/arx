...
import { KeyringService } from "../keyring/service/KeyringService.js";
import type { NamespaceConfig } from "../keyring/service/namespaceConfig.js";
import { encodePayload } from "../keyring/service/vaultPayloadCodec.js";
...
import {
  ...
  SessionImportEnvelopeMissingError,
  SessionVaultMutationUnlockedError,
} from "./errors.js";
import { DEFAULT_AUTO_LOCK_MS } from "./unlock/constants.js";
import { InMemoryUnlockService } from "./unlock/InMemoryUnlockService.js";
import type { UnlockService, UnlockServiceOptions } from "./unlock/types.js";
...
export const createSessionLayer = ({
  messenger,
  vaultMetaPort,
  accountsStore,
  keyringMetas,
  ...
  sessionOptions,
  getIsHydrating,
}: CreateSessionLayerParams): SessionLayer => {
  ...
  const assertVaultLockedAfterMutation = (operation: "initialize" | "loadEnvelope") => {
    if (baseVault.getStatus() === "unlocked") {
      throw new SessionVaultMutationUnlockedError(operation);
    }
  };
  ...
  const importSessionVault = async (envelope: VaultEnvelope): Promise<VaultEnvelope> => {
    ...
    if (!importedEnvelope) {
      throw new SessionImportEnvelopeMissingError();
    }
  ...
  const hydrateVaultMeta = async () => {
    ...
    try {
      ...
      throw new SessionHydrationError("vaultMeta", error);
    }
    ...
    if (meta.payload.envelope) {
      try {
        ...
        throw new SessionHydrationError("vaultEnvelope", error);
      }
      unlock.syncVaultStatus();
      notifySessionStateChanged();
    }
